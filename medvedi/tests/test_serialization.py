...
from medvedi.io import deserialize_df, json_dumps, serialize_df
...
async def test_all_null_rows():
    df = DataFrame({"a": [None, None]})
    assert_frame_equal(df, deserialize_df(serialize_df(df)))


async def test_all_empty_list_rows():
